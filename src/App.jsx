...
import { collection, addDoc, onSnapshot, doc, deleteDoc, serverTimestamp, query, where, orderBy } from 'firebase/firestore'
...
export default function App(){
  ...
  return (
    <div>
      <Header setView={setView} user={user} onLogout={()=>signOut(auth)} />
      <main className="main">
...
function Header({setView,user,onLogout}){
  return <header className="header">
    <div className="logo">👜 <span>DarkStore</span></div>
    <nav>
...
function ProductPage(){ 
  ...
  return <div style={{padding:20}}>
    ...
    <p>Use floating chat to talk to admin for payment.</p>
  </div>
}
...
function Admin({user}){
  const isDemo = localStorage.getItem('demoAdmin')==='true'
  const products = useCollection('products', [orderBy('createdAt','desc')])
  ...
  if(!isDemo && !user) return <div style={{padding:20}}>Login as admin to view</div>
...
function FloatingChat({user}){
  ...
  return <>
    <div className="float" onClick={()=>{ if(!user && !localStorage.getItem('demoAdmin')) return alert('Login first or use demo admin: admin / adminTAZTK'); setOpen(v=>!v) }}>
